        ...
        if(fifo8_status(&keyfifo) + fifo8_status(&mousefifo) + fifo8_status(&timerfifo) + fifo8_status(&timerfifo2) + fifo8_status(&timerfifo3) == 0){
            ...
            if(fifo8_status(&keyfifo) != 0){
                ...
                if(mouse_decode(&mdec, i) != 0){
                    ...
                    sprintf(s, "(%d, %d)", mx, my);
                    boxfill8(buf_back, binfo->scrnx, COL8_008484, 0, 0, 79, 5);
                    putfonts8_asc(buf_back, binfo->scrnx, 0, 0, COL8_FFFFFF, s);
                    sheet_refresh(sht_back, 0, 0, 80, 16);
                    sheet_slide(sht_mouse, mx, my); // preform `sheet_refresh` inside
                }
            }else if(fifo8_status(&timerfifo) != 0){
                i = fifo8_get(&timerfifo); // extract data and make the buffer empty
                io_sti();
                putfonts8_asc(buf_back, binfo->scrnx, 0, 64, COL8_FFFFFF, "10[sec]");
                sheet_refresh(sht_back, 0, 64, 56, 80);
            }else if(fifo8_status(&timerfifo2) != 0){
                i = fifo8_get(&timerfifo2); // extract data and make the buffer empty
                io_sti();
                putfonts8_asc(buf_back, binfo->scrnx, 0, 80, COL8_FFFFFF, "3[sec]");
                sheet_refresh(sht_back, 0, 80, 48, 96);
            }else if(fifo8_status(&timerfifo3) != 0){
                i = fifo8_get(&timerfifo3); // extract data and make the buffer empty
                io_sti();
                if(i != 0){
