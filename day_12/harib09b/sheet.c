...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0){
    ...
    for(h = h0; h <= ctl->top; h++){
        sht = ctl->sheets[h];
        // sid stands for SheetId, which specified by the distance from the head of the sheet array
        sid = sht - ctl->sheets0;
        buf = sht->buf;
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1){
    int h, bx, by, vx, vy, bx0, by0, bx1, by1, pixel_idx;
    ...
    for(h = h0; h <= h1; h++){
        ...
        for(by=by0; by < by1; by++){
            ...
            for(bx=bx0; bx < bx1; bx++){
                vx = sht->vx0 + bx;
                pixel_idx = vy * ctl->xsize + vx;
                if(map[pixel_idx] == sid){
                    vram[pixel_idx] = buf[by * sht->bxsize + bx];
                }
            }
        }
    }
...
void sheet_updown(struct SHEET *sht, int height){
    ...
    if(height < old){
        if(height < 0){ // height < 0 < old (meke the sheet invisible)
            ...
            // draw windows over the current one (height+1)
