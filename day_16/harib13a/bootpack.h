...
void load_tr(int tr); // Task Register
...
struct SHEET {
    unsigned char *buf;
    // col_inv: the color number representing "transparent"
    // height: z-index
...
struct SHTCTL {
    ...
    int xsize, ysize, top; // top: the height(z-index) of the lowest sheet
    struct SHEET *sheets[MAX_SHEETS]; // the order of sheets in sheets0[]
    struct SHEET sheets0[MAX_SHEETS]; // the entities
};
...
struct TIMER {
    struct TIMER *next;
    // timeout: the timepoint when the timer is to be triggered
    unsigned int timeout, flags;
    struct FIFO32 *fifo;
    int data;
};
struct TIMERCTL {
    // next: the next timepoint when the timer control is supposed to take an action
...
struct TASK {
    int sel, flags; // "sel" stands for "selector" in the GDT
    struct TSS32 tss;
};
struct TASKCTL {
    int running; // # of tasks running
    int now; // the selector of the current active task
