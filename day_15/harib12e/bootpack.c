...
void HariMain(void) {
    ...
    load_tr(3 * 8);
    task_b_esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024 - 8; // use the address former by 8 bytes from the tail in order to secure 4 bytes for the first argument of "task_b_main"
    ...
    *((int *) (task_b_esp + 4)) = (int) sht_back; // put the pointer to sht_back right after "task_b_esp" so that "task_b_main" can get it as its first argument
