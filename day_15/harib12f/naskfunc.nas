...
farjump:  ; void farjump(int eip, int cs);
    JMP FAR  [ESP+4]  ; "JMP FAR" instruction loads 4-bytes and 2-bytes succeedingly from the specified address, and put them into EIP and CS respectively
    RET    ; go back to the line at which taskswitch4() is called
